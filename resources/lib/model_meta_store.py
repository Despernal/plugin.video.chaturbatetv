...
def bio_full_plot_for_view_info(
    ...
    """Rich profile rendering for the v0.7.25 view-info directory.

    Superset of plot_for_offline_row: every line that one renders, this
    one renders, plus the bio_* columns we get from biocontext (sex,
    body type, fan club info, wish list, about me, social medias).

    Sparse rows omit lines they don't have data for -- no empty
    "Body:" or "Fan club:" placeholders. The result feeds straight
    into a ListItem.setInfo("video", {"plot": ...}) so Kodi displays
    it in the right pane while the user is on the Profile entry.
    """
    parts: list[str] = []
